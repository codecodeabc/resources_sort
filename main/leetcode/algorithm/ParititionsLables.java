 ...
 */
public class ParititionsLables {


    public static  List<Integer> partitionLabels(String S) {
        char[] chars = S.toCharArray();
        if (chars.length <= 1) {
            return Arrays.asList(chars.length);
        }
        ArrayList<Integer> list = new ArrayList<>();
        int sum = 0;
        for (int i = 0,flag = i; i < chars.length;i++) {
            for (int j = chars.length - 1; j >= flag; j--) {
                if(chars[i] == chars[j]){
                    if(j >= flag){
                        flag = j;
                        break;
